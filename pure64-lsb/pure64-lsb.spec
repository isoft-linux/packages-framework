...
Patch1: pure64-lsb-lsb_start_daemon-fix.patch
Patch2: pure64-lsb-trigger.patch
Patch3: pure64-lsb-arm.patch
Patch4: pure64-lsb-aarch64.patch
License: GPLv2
...
Requires: pure64-lsb-core%{?_isa} = %{version}-%{release}
Requires: pure64-lsb-cxx%{?_isa} = %{version}-%{release}
Requires: pure64-lsb-desktop%{?_isa} = %{version}-%{release}
Requires: pure64-lsb-languages = %{version}-%{release}
Requires: pure64-lsb-printing = %{version}-%{release}
#Requires: pure64-lsb-trialuse = %{version}-%{release}
...
Requires: pure64-lsb-submod-security%{?_isa} = %{version}-%{release}

Provides: lsb-core-%{archname} = %{version}-%{release}
Provides: lsb-core-noarch = %{version}-%{release}
#Obsoletes: pure64-lsb < %{version}-%{release}
...
Requires: pure64-lsb-submod-multimedia%{?_isa} = %{version}-%{release}
Requires: pure64-lsb-core%{?_isa} = %{version}-%{release}
...
Obsoletes: pure64-lsb-graphics < %{version}-%{release}
...
Obsoletes: pure64-lsb-printing < %{version}-%{release}
...
# According to https://bugzilla.pure64.com/show_bug.cgi?id=232918 , the '-static' option
# is imported against segfault error while running pure64_lsb_trigger
%ifarch %{arm}
gcc $RPM_OPT_FLAGS -Os -fno-stack-protector -o pure64_lsb_trigger{.%{_target_cpu},.c} -DLSBSOVER='"%{lsbsover}"' \
  -DLDSO='"%{ldso}"' -DLSBLDSO='"/%{_lib}/%{lsbldso}"' -D_GNU_SOURCE
%else
gcc $RPM_OPT_FLAGS -Os -static -fno-stack-protector -o pure64_lsb_trigger{.%{_target_cpu},.c} -DLSBSOVER='"%{lsbsover}"' \
  -DLDSO='"%{ldso}"' -DLSBLDSO='"/%{_lib}/%{lsbldso}"' -D_GNU_SOURCE
%endif
install -p -m 700 pure64_lsb_trigger.%{_target_cpu} \
  $RPM_BUILD_ROOT%{_sbindir}/pure64_lsb_trigger.%{_target_cpu}

cp -p pure64_lsb_init $RPM_BUILD_ROOT/bin/pure64_lsb_init

%triggerpostun -- glibc
if [ -x /usr/sbin/pure64_lsb_trigger.%{_target_cpu} ]; then
  /usr/sbin/pure64_lsb_trigger.%{_target_cpu}
fi
...
/%{_lib}/*so*
/lib/lsb*
%{_sbindir}/pure64_lsb_trigger.%{_target_cpu}
